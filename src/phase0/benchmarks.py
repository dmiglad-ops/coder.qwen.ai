...
def bench_buyhold(df: pd.DataFrame, fee=0.001, slip=0.0005) -> dict:
    ...
    ret = df["close"].iloc[-1] / df["close"].iloc[0] - 1 - 2 * fee - 2 * slip
    eq = df["close"].to_numpy() / df["close"].to_numpy()[0]
    peak = np.maximum.accumulate(eq)
    return {
        "total_return": float(ret),
        "max_drawdown": float(abs((eq / peak - 1).min())),
...
def bench_sma_cross(df: pd.DataFrame, fast=20, slow=50, fee=0.001, slip=0.0005) -> dict:
    """Бенчмарк 4: пересечение SMA 20/50; сравнение ПО SHARPE."""
...
def bench_random_timing(df: pd.DataFrame, model_preds: np.ndarray, n_runs: int = 300,
                        fee=0.001, slip=0.0005, holding: int = 24, seed: int = 7) -> dict:
...
def bench_random_direction(df: pd.DataFrame, model_preds: np.ndarray, n_runs: int = 300,
                           fee=0.001, slip=0.0005, holding: int = 24, seed: int = 11) -> dict:
...
def run_all_benchmarks(df: pd.DataFrame, model_preds: np.ndarray, model_return: float,
                       n_runs: int = 300, fee=0.001, slip=0.0005, holding=24) -> dict:
